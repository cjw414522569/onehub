...
const REQUIRED_TOKENS = [
  'pub enum AddressFamily', 'pub enum ProxyKind', 'pub struct HopPolicy', 'pub struct ProxyHop',
  'pub enum ChainValidation', 'pub struct ProxyChain', 'pub fn validate', 'pub fn proxy_jump',
  'pub fn proxy_jump_multi', 'cycle_detection_holds_for_all_small_chains',
  'cycle_is_detected_when_host_revisited',
];
...
for (const kind of ['Direct', 'JumpHost', 'Socks5', 'HttpConnect']) {
  if (!sourceText.includes(kind)) errors.push(`ProxyKind is missing variant: ${kind}`);
}
...
if (errors.length > 0) {
  console.error(`proxy chain contract failed with ${errors.length} error(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log('proxy chain contract valid: ProxyJump/multi-hop/SOCKS/HTTP, per-hop policy, cycle detection property test, cargo check/test --locked passed.');