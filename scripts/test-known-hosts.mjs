...
const REQUIRED_TOKENS = [
  'pub fn hostname_matches_pattern', 'pub fn hashed_host_matches', 'pub fn host_field_matches',
  'pub struct KnownHostsStore', 'pub fn from_text', 'pub fn lookup',
  'wildcard_pattern_matches_openssh_semantics',
  'hashed_host_matches_openssh_vector',
  'bracket_port_rule_matches',
  'open_ssh_compat_corpus',
];
...
for (const marker of ['|1|', 'HMAC-SHA1', 'ct_eq']) {
  if (!sourceText.includes(marker)) errors.push(`known_hosts must include marker/token: ${marker}`);
}
...
if (errors.length > 0) {
  console.error(`known_hosts contract failed with ${errors.length} error(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log('known_hosts contract valid: reading, hashed hosts (HMAC-SHA1 vector), wildcards, port rules, OpenSSH compat corpus, cargo check/test --locked passed.');